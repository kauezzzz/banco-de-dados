...
SELECT titulo,carga_horaria, (carga_horaria * 0.25) FROM cursos;
```
...
SELECT COUNT(area_de_atuacao) AS "Quantidade de professores" FROM professores WHERE area_de_atuacao LIKE "%desenvolvimento";
```
...
COUNT(alunos.curso.id) AS "QNTD Alunos"
FROM alunos
INNER JOIN cursos
